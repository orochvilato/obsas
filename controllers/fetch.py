...
def update_acteurs_stats():
    ...
    for itv in interventions:
        if not itv['acteur'] in compteitv.keys():
            ...
            compteitv[itv['acteur']] += 1
    rankings = {'exprimepct':[],'disspct':[],'votefipct':[],'voteempct':[]}
    ...
    for dep in deputes:
        ...
        for stat in ['exprimepct','votefipct','voteempct','disspct']:
            rankings[stat].append((dep['uid'],stats[stat]))
        dep['statsvote'] = stats
    ...
    # classements
    for stat in ['exprimepct','votefipct','voteempct','disspct']:
        rankings[stat] = dict((p[0],1+i) for i,p in enumerate(sorted(rankings[stat],key=lambda x:x[1], reverse=True)))
    
    for dep in mdb.acteurs.find():
        ranks = {}
        for stat in ['exprimepct','votefipct','voteempct','disspct']:
            ranks[stat] = rankings[stat][dep['uid']]
        mdb.acteurs.update({'uid':dep['uid']},{'$set':{'rankings':ranks}})
