...
def depute():
    ...
    for s in scrutins:
        ...
        if _mots and _mots['mots']:
            ...
            mx = mots[0][1]
            mots = [ [mot,int(100*float(count)/mx)] for mot,count in mots if not mot in nuages_excl]
        else:
            mots = []
...
def nuages():
    ...
    for m in _mots:
        for mot,n in m['mots']:
            if not mot in nmots.keys():
                nmots[mot] = 1
            else:
                nmots[mot] += 1
    return dict(nmots=sorted(nmots.items(),key=lambda x:x[1], reverse=True)[:300])
