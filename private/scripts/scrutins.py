...
def normalize(s):
    return strip_accents(s).replace(' ','').replace('-','').replace(u'\x0a','').lower() if s else s
...
class ScrutinsSpider(scrapy.Spider):
    ...
    def parse_scrutin(self, response):
        positions =['Pour','Contre','Abstention','Non-votants']
        ...
        for pos in positions:
            p = response.xpath('//div[@class="%s"]/ul[@class="deputes"]/li' % pos)
            for dep in p:
